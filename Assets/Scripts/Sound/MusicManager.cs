...
namespace NSMB.Sound {
    public unsafe class MusicManager : QuantumSceneViewComponent<StageContext> {
        ...
        [SerializeField] private Songinator midiMusicIceRunPlayer;
        ...
        [SerializeField] private bool usesMidi = false;
        ...
        public void OnValidate() {
            ...
            this.SetIfNull(ref midiMusicIceRunPlayer);
        ...
        public void HandleMusic(QuantumGame game, bool force) {
            ...
            if (!force && !(musicPlayer.IsPlaying || AnyMusicPlaying())) {
                return;
            }
            ...
            if (gamemode is IceRunGamemode) {
                midiMusicIceRunPlayer.SetPlaybackState(Songinator.PlaybackState.PLAYING);
            } else if (mega) {
                if (usesMidi) SetMusicType(MusicType.Mega); else musicPlayer.Play(f.FindAsset(stage.MegaMushroomMusic));
            } else if (invincible) {
                if (usesMidi) SetMusicType(MusicType.Invincibility); else musicPlayer.Play(f.FindAsset(stage.InvincibleMusic));
            } else {
